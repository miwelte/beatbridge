...
# PLL
LOCK_WINDOW_S = 0.070  # a bit wider for reliability
PLL_ALPHA = 0.18
PLL_BETA = 0.02

# Tempo estimation (robust)
ONSET_HISTORY_S = 12.0
IOI_MIN_S = 0.12   # allow denser hits; folding will handle it
IOI_MAX_S = 1.80
BPM_BIN_SIZE = 1.0
...
PEAK_HOLD_DECAY_PER_S = 0.175  # slower decay => longer hold
...
class TempoEstimator:
    ...
    def estimate_bpm(self) -> Optional[float]:
        ...
        # Vote in BPM domain, include 0.5x/1x/2x candidates
        votes = {}
        raw_vals = []
        for dt in iois:
            base = 60.0 / dt
...
def build_panel(
    ...
    # Hold marker color depends on CURRENT peak (not hold)
    hold_color = "green" if peak < PEAK_HOT_THRESHOLD else "red"

    # IMPORTANT: RMS and PEAK meters use SAME normalization (raw 0..1),
    # so RMS can never exceed PEAK visually.
    tbl.add_row(
        "RMS",
        make_meter(rms, rms_hold, METER_WIDTH, hold_color="green")
        + f"  {rms:0.4f} ({rms_max:0.4f})"
    )
    tbl.add_row(
        "PEAK",
        make_meter(peak, peak_hold, METER_WIDTH, hold_color=hold_color)
        + f"  {peak:0.4f} ({peak_max:0.4f})"
    )

    bpm_txt = f"{bpm:0.2f}" if bpm is not None else "--"
    est_txt = f"{bpm_est:0.2f}" if bpm_est is not None else "--"
    tbl.add_row("BPM", f"[bold]{bpm_txt}[/bold]   est:{est_txt}   locks:{locks}")

    tbl.add_row("Noise", f"{noise_floor:0.6f}")
    tbl.add_row("Thresh", f"{thr:0.6f}")
...
def main() -> None:
    ...
    with sd.InputStream(
        ...
        while True:
            ...
            # sanity: RMS must never exceed PEAK (raw)
            # if it ever does, something is fundamentally wrong upstream.
            if rms_raw > peak_raw + 1e-6:
                console.log(f"[red]SANITY FAIL rms({rms_raw:0.5f}) > peak({peak_raw:0.5f})[/red]")

            # smooth for UI
            rms_smooth = 0.85 * rms_smooth + 0.15 * rms_raw
            peak_smooth = max(peak_smooth * 0.6, peak_raw)
            ...
            # peak-hold (slower decay)
            rms_hold = max(rms_hold - PEAK_HOLD_DECAY_PER_S * dt, rms_smooth)
            peak_hold = max(peak_hold - PEAK_HOLD_DECAY_PER_S * dt, peak_smooth)
            ...
            for (t_evt, strength) in onsets:
                ...
                if bpm_est is not None:
                    ...
                    if locked:
                        console.log(f"[green]LOCK[/green] t={t_evt - t0:7.3f}s  bpm={pll.stable_bpm():0.2f}")

            # Always show some BPM visibility in console at least every ~1.5s
            if now - last_bpm_print_t > 1.5:
                last_bpm_print_t = now
                console.log(
                    f"BPM_EST={bpm_est if bpm_est is not None else 'None'}  "
                    f"PLL_BPM={pll.stable_bpm() if pll.stable_bpm() is not None else 'None'}  "
                    f"locks={pll.locks}"
                )
