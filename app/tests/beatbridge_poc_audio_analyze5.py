...
# Onset function
DIFF_LP_HZ = 18.0
DIFF_THRESH_FACTOR = 5.0       # a bit lower for more consistent triggers
DIFF_THRESH_MIN = 2e-4
REFRACTORY_S = 0.110
...
IOI_MAX_S = 2.00
BPM_BIN_SIZE = 1.0
MIN_ONSETS_FOR_BPM = 6
MAX_GAP_K = 4                # <--- key fix: use multi-gap IOIs

# UI
METER_WIDTH = 44
PEAK_HOT_THRESHOLD = 0.80
PEAK_HOLD_DECAY_PER_S = 0.175
...
class KickOnsetDetectorDiff:
    ...
    diff_floor: float = 1e-4          # initialize non-zero
    ...
    def process_block(self, mono: np.ndarray, block_start_time: float) -> Tuple[List[Tuple[float, float]], float, float]:
        ...
        # Robust floor: use percentile (not median), then slow EMA.
        # Percentile avoids collapsing to 0 in "mostly zeros" diff.
        p = float(np.percentile(diff_s, 75))  # activity baseline
        p = max(p, DIFF_THRESH_MIN)

        self.diff_floor = 0.995 * self.diff_floor + 0.005 * p
        self.diff_floor = max(self.diff_floor, DIFF_THRESH_MIN)
...
# -----------------------------
# Tempo estimator (multi-gap IOI voting)
# -----------------------------
@dataclass
class TempoEstimator:
    ...
    def estimate_bpm(self) -> Optional[float]:
        ...
        times = self.onset_times[-40:]  # short but enough
        votes = {}
        candidates = []

        # Multi-gap IOIs: dt = t[i] - t[i-k], k=1..MAX_GAP_K
        for i in range(1, len(times)):
            for k in range(1, min(MAX_GAP_K, i) + 1):
                dt = times[i] - times[i - k]
                if not (IOI_MIN_S <= dt <= IOI_MAX_S):
                    continue

                base = 60.0 / dt
                # If dt spans k beats, then bpm ~ base * k
                # Also include half/double robustness.
                for mult in (0.5, 1.0, 2.0):
                    bpm = self._fold_bpm(base * k * mult)
        ...
        # choose best bin by votes; if tie -> prefer higher BPM (avoids half-time)
        best_count = max(votes.values())
        best_bins = [b for b, c in votes.items() if c == best_count]
        best_bin = max(best_bins)  # bias to higher tempo on ties

        close = [b for b in candidates if abs(b - best_bin) <= 1.5 * BPM_BIN_SIZE]
        return float(np.median(close)) if len(close) >= 5 else float(best_bin)
...
def build_panel(
    ...
    return Panel(tbl, title="Beatbridge PoC (Kick Diff-Onset + BPM v5)", border_style="cyan")
